...
export const getEpisodesToMatch = async (): Promise<GetEpisodesToMatchResult> => {
  ...
  const firstVideo = pendingVideosResponse.Items?.[0] as
    | Pick<
        VideoEntity,
        'primaryKey' | 'updatedAt' | 'myAnimeListId' | 'dub' | 'episode' | 'matchingStatus' | 'matchingGroup'
      >
    | undefined;
  if (!firstVideo) {
    return [];
  ...
  const groupVideos = (await docClient.send(
    new ScanCommand({
      TableName: config.value.database.tableName,
      IndexName: config.value.database.matcherSecondaryIndexName,
      FilterExpression:
        'matchingGroup = :group AND (#S = :pending OR (#S = :failed AND #matchingPerformedAttempts < :maxAttempts AND #updatedAt < :retryThreshold))',
      ExpressionAttributeNames: {
        '#S': 'matchingStatus',
        '#matchingPerformedAttempts': 'matchingPerformedAttempts',
        '#updatedAt': 'updatedAt',
